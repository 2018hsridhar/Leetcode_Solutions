...
Max depth = max depth of ANY integer -> so go thru list initialy and already compute for this as a first step!
Then proceed with the rest of the DFS algo
It's just another dfs step honestly
...
class Solution 
{
    ...
    {
        int dSum = 0;
        int maxDepthOfInt = findMaxDepth(nestedList);
        // System.out.printf("Max depth of nested list = %d\n", maxDepthOfInt);
        dSum = helper(nestedList, 1, maxDepthOfInt);
        return dSum;    
    }
    ...
    public int findMaxDepth(List<NestedInteger> nestedList)
    {
        ...
        {
            if(!x.isInteger())
                max_depth = Math.max(max_depth, 1 + findMaxDepth(x.getList()));
        }
        return max_depth;
    }
    
    public int helper(List<NestedInteger> nestedList, int depth, int maxDepthOfInt)
    {
        int prod_of_level = 0;
        ...
        {
            if(x.isInteger())
                prod_of_level += ((maxDepthOfInt - depth + 1) * x.getInteger());
            else
                prod_of_level += helper(x.getList(), depth + 1, maxDepthOfInt); 
